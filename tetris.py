...
COLORS = ['gray', 'lightgreen', 'yellow', 'blue', 'orange', 'purple', 'pink']
...
class Tetris():
    ...
    def reset_tetrio(self):
        self.tetrio = random.choice(Tetris.TETRIO)[:]
        self.tetrio_color = random.randint(1, len(COLORS) - 1)
        self.tetrio_offset = [-2, Tetris.FIELD_WIDTH // 2]
        self.game_over = any(not self.is_cell_free(r, c) for (r, c) in self.get_tetrio_coords())

    def get_tetrio_coords(self):
        return [(r + self.tetrio_offset[0], c + self.tetrio_offset[1]) for (r, c) in self.tetrio]

    def apply_tetrio(self):
        for (r, c) in self.get_tetrio_coords():
            self.field[r][c] = self.tetrio_color
    ...
    def get_color(self, r, c):
        return self.tetrio_color if (r, c) in self.get_tetrio_coords() else self.field[r][c]
    ...
    def move(self, dr, dc):
        with self.move_lock:
            ...
            if all(self.is_cell_free(r + dr, c + dc) for (r, c) in self.get_tetrio_coords()):
                self.tetrio_offset = [self.tetrio_offset[0] + dr, self.tetrio_offset[1] + dc]
            elif dr == 1 and dc == 0:
                self.game_over = any(r < 0 for (r, c) in self.get_tetrio_coords())
    ...
    def rotate(self):
        with self.move_lock:
            ...
            ys = [r for (r, c) in self.tetrio]
            xs = [c for (r, c) in self.tetrio]
            size = max(max(ys) - min(ys), max(xs) - min(xs))
            rotated_tetrio = [(c, size - r) for (r, c) in self.tetrio]
            wallkick_offset = self.tetrio_offset[:]
            tetrio_coord = [(r + wallkick_offset[0], c + wallkick_offset[1]) for (r, c) in rotated_tetrio]
            min_x = min(c for r, c in tetrio_coord)
            max_x = max(c for r, c in tetrio_coord)
            max_y = max(r for r, c in tetrio_coord)
            ...
            tetrio_coord = [(r + wallkick_offset[0], c + wallkick_offset[1]) for (r, c) in rotated_tetrio]
            if all(self.is_cell_free(r, c) for (r, c) in tetrio_coord):
                self.tetrio, self.tetrio_offset = rotated_tetrio, wallkick_offset
